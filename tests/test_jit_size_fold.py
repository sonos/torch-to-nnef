...
skipif_torch_lt_20 = pytest.mark.skipif(
    ...
    reason=(
        "torch < 2.0 lacks the shape-analysis behaviour the fold relies on "
        "to evaluate `aten::dim/len` against the example inputs"
    ),
)
...
def test_dim_in_if_condition_folds():
    ...
    x = torch.randn(2, 4)
    assert _node_count(m.graph, ATEN_DIM) == 1
    ...
    assert folded == 1
    assert _node_count(m.graph, ATEN_DIM) == 0
...
def test_len_in_if_condition_folds():
    ...
    x = torch.randn(1, 5)
    assert _node_count(m.graph, ATEN_LEN) == 1
    ...
    assert folded == 1
    assert _node_count(m.graph, ATEN_LEN) == 0
...
def test_size_consumed_by_tensor_op_is_not_folded():
    ...
    x = torch.randn(3, 5)
    assert _node_count(m.graph, ATEN_SIZE_KIND) == 1
...
def test_numel_as_scalar_is_not_folded():
    ...
    x = torch.randn(2, 3, 4)
    assert _node_count(m.graph, ATEN_NUMEL) == 1
