...
skipif_torch_lt_20 = pytest.mark.skipif(
    ...
    reason=(
        "torch < 2.0's `_jit_pass_inline` + `_jit_interpret_graph` interplay "
        "leaves the If structure in a form `fold_data_dependent_ifs` cannot "
        "evaluate; the pass itself still runs as a no-op on those graphs"
    ),
)
