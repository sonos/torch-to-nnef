...
from torch_to_nnef.torch_graph.torch_const import DATA_KIND, IF_KIND
...
@pytest.mark.skipif(
    ...
    reason=(
        "torch < 2.0 lacks the shape-analysis behaviour "
        "`replace_size_calls_with_constants` relies on to fold "
        "`aten::dim` from a 2D input"
    ),
)
...
def test_strip_prim_data():
    ...
    assert _node_count(m.graph, DATA_KIND) == 1
    n_stripped = strip_prim_data(m.graph)
    torch._C._jit_pass_dce(m.graph)
    assert n_stripped == 1
    assert _node_count(m.graph, DATA_KIND) == 0


@pytest.mark.skipif(
    condition=torch_version() < "2.0.0",
    reason="same shape-analysis pre-condition as the per-pass test above",
)
