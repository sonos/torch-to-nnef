...
### Added
- **Scoped residency and asynchronous prefetch for `OffloadedTensor`** (core): `TensorResidencyPool` keeps disk-backed tensors materialized across repeated operations, supports read and read-write leases, background lookahead loading, and a `max_cached_bytes` cache budget. `ResidencyStrategy.LRU` provides least-recently-used retention with optional manual `pin` and `unpin` overrides. `ResidencyStrategy.FIXED` avoids cyclic-scan churn by preserving the first completed values that fit while later values stream without displacing them; manual pin operations are intentionally LRU-only. A context-local residency scope transparently serves ordinary `OffloadedTensor.reload` calls, so existing model and quantizer code can benefit without replacing parameters or acquiring each tensor explicitly. Callers retain explicit `prefetch`, `acquire`, `flush`, and `evict` controls where traversal policy knows the next or final consumer. `OffloadedTensor.reload(device=...)` can select a device for one materialization without mutating the tensor's configured target device.

### Fixed
- **Independent offloaded tensors with the same logical name no longer overwrite or delete each other's payloads.** Each tensor receives a distinct storage identity, so a stale object's destructor cannot unlink a newer model or calibration run's file when both reuse the same offload directory, name, and dtype.
...
- **One shared gated-delta surface** (core): the tract `tract_transformers_gdn_recurrent` contract (operand layout, version gate, head-dim and dtype constraints), the torch reference recurrence and the q/k `l2norm` now live once in `torch_to_nnef/op/gated_delta.py` instead of once per export path. Behaviour is unchanged; the point is that a second emitter (a reified GDN module captured by a `ModuleInfoExtractor`, which has no portable scan fallback) can reuse the same gate rather than emit an operator the pinned tract cannot load.
...
- **`examples/nuextract3`** (LLM): NuExtract3 image-to-Markdown export example
  stacked on the Qwen3.5 dense VLM support, with a minimal Rust + tract runner
...
### Fixed
- **An `eval_symbols`-pinned axis (e.g. `TARGETS__TIME` for single-step RNN-T decoding) no longer ships a permanently vestigial `tract_assert`.** `prepare_subnet_export` computed and filtered the auto-generated `tract_assert <SYMBOL> >= 1` extensions *before* pinning eval-symbol axes out of `dyn`, so the "is this symbol still dynamic" filter always saw the axis as still present and kept the assertion, even though the axis becomes a static dim in every tensor shape once pinned. Every NeMo ASR decoder export using `eval_symbols` (the documented idiom for single-step decoding, e.g. the shipped `shapes.parakeet.yaml`) carried one of these. Harmless on its own, but `tract >= 0.23.4` added a diagnostic for exactly this shape (see below) that t2n's own error handling then treated as fatal. A user/slug/derived-declared `extensions:` entry naming a symbol `eval_symbols` just pinned static is now dropped too (new `drop_assertions_referencing_symbols` in `torch_to_nnef/remodeler/dyn_axes.py`), logged when it fires. This is scoped to that one case specifically, since requiring every identifier in a free-form assertion to be a known axis would wrongly drop a legitimately labeled one (e.g. the LLM exporter's `tract_assert tg: S==1`).
- **`tract >= 0.23.4`'s new "mislabeled symbol name" assertion warning no longer fails an otherwise-successful `check_io` run.** That tract release started warning when an NNEF `tract_assert` constrains a symbol absent from every tensor shape (harmless, tract itself says "it has no effect"), but t2n's `tract_err_filter` treated any leftover stderr as fatal and raised `T2NErrorTract` on it. The pattern is now filtered like the other benign deprecation warnings, as defense in depth for any other case that still emits it (e.g. an actually mislabeled symbol in a hand-written shape-config). `tract_err_filter` also no longer drops a filtered stderr line silently: every pattern it recognizes (this one and the pre-existing deprecation warnings) is re-emitted through t2n's own logger at `WARNING` level, so filtering only means "don't fail the export on this," not "hide it."
