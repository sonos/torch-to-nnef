...
### Fixed
- **An `eval_symbols`-pinned axis (e.g. `TARGETS__TIME` for single-step RNN-T decoding) no longer ships a permanently vestigial `tract_assert`.** `prepare_subnet_export` computed and filtered the auto-generated `tract_assert <SYMBOL> >= 1` extensions *before* pinning eval-symbol axes out of `dyn`, so the "is this symbol still dynamic" filter always saw the axis as still present and kept the assertion, even though the axis becomes a static dim in every tensor shape once pinned. Every NeMo ASR decoder export using `eval_symbols` (the documented idiom for single-step decoding, e.g. the shipped `shapes.parakeet.yaml`) carried one of these. Harmless on its own, but `tract >= 0.23.4` added a diagnostic for exactly this shape (see below) that t2n's own error handling then treated as fatal. The same "still present in `dyn`" drop-filter (`filter_assertions_present_in_dyn`, new in `torch_to_nnef/remodeler/dyn_axes.py`) now also runs over user/slug/derived-declared `extensions:` entries, not just the auto-generated ones, so a hand-written or slug-registry bound referencing a pinned/renamed-away symbol is dropped the same way instead of leaking through unfiltered.
