...
### Added
- **Scoped residency and asynchronous prefetch for `OffloadedTensor`** (core): `TensorResidencyPool` keeps disk-backed tensors materialized across repeated operations, supports read and read-write leases, optional `max_cached_bytes` LRU retention, fixed-resident pins, and background lookahead loading. Pins let cyclic workloads preserve a chosen hot subset while the remaining cache uses LRU. A context-local residency scope transparently serves ordinary `OffloadedTensor.reload` calls, so existing model and quantizer code can benefit without replacing parameters or acquiring each tensor explicitly. Callers retain explicit `prefetch`, `pin`, `unpin`, `acquire`, `flush`, and `evict` controls where traversal policy knows the next or final consumer. `OffloadedTensor.reload(device=...)` can select a device for one materialization without mutating the tensor's configured target device.
