...
def _rich_classes(rich):
    ...
    """
    base = rich.__name__
    table_cls = import_module(f"{base}.table").Table
    text_cls = import_module(f"{base}.text").Text
    console_cls = import_module(f"{base}.console").Console
    columns_cls = import_module(f"{base}.columns").Columns
    rule_cls = import_module(f"{base}.rule").Rule
    return table_cls, text_cls, console_cls, columns_cls, rule_cls


def _make_tables(rich, rep: SubnetSignature):
    table_cls, _, _, _, _ = _rich_classes(rich)
...
def _print_diffs(rich, console, groups):
    table_cls, text_cls, _, _, _ = _rich_classes(rich)
...
def print_signatures_rich(
    ...
    *,
    ...
    _, text_cls, console_cls, columns_cls, rule_cls = _rich_classes(rich)
    console = console_cls()
    if model_label:
