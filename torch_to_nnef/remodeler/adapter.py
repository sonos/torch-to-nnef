    ...
    *outputs_keep* against the **flattened** output names and filters
    ...
    def __init__(
        ...
        *,
        ...
        # -- Output flattening (deferred to forward, zero init cost) ---------
        # When outputs_keep is set, outputs are flattened at each forward()
        # call and filtered by name.  No forward pass needed at init — the
        # flat names come from the config (generated by --dump-shape-config)
        # and are already validated by validate_registry_against_signatures.
        self._outputs_keep_set: T.Optional[set[str]] = (
            set(self._outputs_keep) if self._outputs_keep else None
        )

    # ------------------------------------------------------------------
    # Input collapse / bind / external interface
    # ------------------------------------------------------------------
    ...
    def forward(self, *args, **kwargs):
        ...
        if self._outputs_keep_set is None:
            return outs
        ...
        kept = [
            ...
            if n in self._outputs_keep_set
        ]
        return tuple(kept)
