...
def _conv(packed_params, node, g, name_to_tensor):
    ...
    out_tensor_name = f"{node.export_name}_conv"
    ...
    NOperation(
        graph=g,
        type="conv",
        name=f"{node.export_name}_conv",
        inputs=(
            name_to_tensor[node.export_inputs[0]],
            weight_ref,
            bias_ref,
        ),
...
def conv1d_relu(g, node, name_to_tensor, null_ref, torch_graph):
    packed_params = torch_graph.get_node_by_export_name(
        node.export_inputs[1]
    ).attributes['ref_module']
...
def quantized_node_to_nnef_tensor_and_ops(
    ...
    familly, op_name = node.kind.split("::")
    assert familly == "quantized"
    globals()[op_name](
        g=g,
        node=node,
        name_to_tensor=name_to_tensor,
