...
class TractCheckTolerance(str, enum.Enum):
    """level tolerated difference between original output values and those generated by tract
...
class TractNNEF(InferenceTarget):
    ...
    def __init__(
        ...
        Args:
            ...
            dynamic_axes: Optional specification of dynamic dimension
                ...
                set dynamic_axes to a dict with schema:
                    ...
                    VALUE (dict or list): If a dict, keys are axis indices
                        ...
                        an axis index.
            specific_tract_binary_path: filepath of tract cli in case of custom non released version of tract (for testing purpose)
            check_io_tolerance: TractCheckTolerance
                level of difference tolerance between original output values and those generated by tract (those are defined tract levels)
            specific_properties: custom tract_properties you wish to add inside NNEF asset (will be parsed by tract as metadata)
            dump_identity_properties: add tract_properties relative to user identity (host, username, OS...), helpfull for debug
            force_attention_inner_in_f32:
                    control if attention should be forced as f32 inside (even if inputs are all f16),
            ...
            enable_sdpa: enable the conversion of scaled_dot_product_attention as a tract operator (intead of a NNEF fragment)
        """
...
def debug_dumper_pytorch_to_onnx_to_nnef(
    ...
    """Try to export same model with ONNX (and convert the onnx to nnef via tract).
...
def assert_io_and_debug_bundle(
    ...
    """Core check function to ensure tract give same output as PyTorch within bounds"""
    with tempfile.TemporaryDirectory() as tmpdir:
        try:
            if io_npz_path is None:
                io_npz_path = Path(tmpdir) / "io.npz"
