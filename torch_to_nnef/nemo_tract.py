...
def decoder_fix_input_example_batch_size(
    ...
    NeMo decoder batch size option input example is wrong.
    This function adjusts the input example to have the specified batch size.

    Args:
        input_example: The original input example tuple (input_ids, ...).
    ...
    *,
    ...
    Args:
        ...
        apply_sequential_examples: If True, use sequential input examples
            for each subnet.
        batch_size: Batch size to use for dummy input generation.
...
class CollapseBatchDimWrapper(torch.nn.Module):
    ...
    def __init__(
        ...
        for idx, name in enumerate(self._orig_input_names):
            ...
            rank = 0
            if (
                inner_ex is not None
                and idx < len(inner_ex)
                and torch.is_tensor(inner_ex[idx])
            ):
    ...
    def input_example(self, max_batch: int = 1):
        ...
        for name, tensor in zip(self._orig_input_names, inner):
            ...
            if torch.is_tensor(tensor):
                ...
                expected_rank = sum(
                    1 for s in self._full_axes_spec.get(name, []) if s != "B"
                )
