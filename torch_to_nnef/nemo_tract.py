...
def iter_nemo_model_subnets(
    ...
    Args:
        ...
        split_joint_decoder: Whether to split joint decoder subnets (if encountered).
        remove_unused_inputs: Whether to remove unused inputs from subnet exports.
    ...
    for subnet_name in model.list_export_subnets():
        ...
        with exportable_nemo_net(
            ...
            if subnet_name == "decoder_joint":
                # TODO: remove_unused_inputs with decoder_joint & decoder
                if split_joint_decoder:
                    ...
                    yield (
                        "decoder",
                        subnet.decoder,
                        subnet.decoder.input_example(),
                        subnet.decoder.dynamic_shapes_for_export(False),
                    )
                    yield (
                        "joint",
                        subnet.joint,
...
def export_nemo_asr_model(
    ...
    **kwargs,
    ...
    Args:
        ...
        float_dtype: Optional float dtype to use for export.
        remove_unused_inputs: Whether to remove unused inputs from the exported model.
            This happen for decoder/decoder+joint subnets that do not use target_length.
...
class WrapDecoder(torch.nn.Module):
