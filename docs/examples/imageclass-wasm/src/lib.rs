...
fn predict_class_internal(image_base64: &str) -> TractResult<Prediction> {
    ...
    let classes: Vec<&str> = classes_txt.split("\n").collect();
    ...
    Ok(Prediction {
        score: best.0,
        class_id: best.1,
        label: classes[best.1 as usize].to_string(),
    })
}

#[wasm_bindgen]
pub fn predict_class(image_base64: &str) -> Result<JsValue, JsError> {
    let prediction_res = predict_class_internal(image_base64);
