"""Export numind/NuExtract3 (Qwen3.5 dense VLM, ``qwen3_5``) for the Rust demo.
...
Produces, in ``--out`` dir:
  ...
  * ``nuextract3.json``   : shapes + per-layer state layout for the runtime.
  * ``tokenizer.json``    : tokenizer for the Rust runner to decode Markdown.
  * ``*.dat``             : sample input_ids + position_ids + pixel_values.

Usage:
    python export.py --dummy --out ./exp     # tiny random model, no download
    python export.py --image receipt.png --out ./exp
"""
...
# memory (NuExtract3 is large); the Rust demo casts each input to whatever
# dtype the graph expects, so f16 needs no runtime change there.
...
def _manifest_layers(text_conf):
    """Per-layer state layout the Rust runtime reads from ``nuextract3.json``.
...
def _write_manifest_and_inputs(args, model, s, layout, dec_in, dec_out):
    """Write ``nuextract3.json`` + sample inputs as NNEF ``.dat`` tensors."""
    conf = model.config
    tc = conf.text_config
    manifest = {
        "repo": "dummy" if args.dummy else args.repo,
    ...
    (args.out / "nuextract3.json").write_text(json.dumps(manifest, indent=2))
    ...
    print(f"[nuextract3] wrote nuextract3.json + *.dat to {args.out}")
    print(f"[nuextract3] seq={s.seq} img_tokens={n_img} layers={len(layout)}")
...
def main() -> None:
    ...
    ap.add_argument(
        "--prompt",
        default="Convert this document image to Markdown.",
        help="User prompt paired with the image sample.",
    )
    ...
    if args.dummy or args.repo is None:
        print(f"[nuextract3] building tiny dummy qwen3_5 model ({args.dtype})")
        model = Qwen3_5ForConditionalGeneration(_dummy_config()).eval()
        model = model.to(dtype)
        processor = None
    else:
        if args.image is None:
            raise SystemExit("--image is required for real NuExtract3 export")
        print(f"[nuextract3] loading {args.repo} ({args.dtype})")
        model = Qwen3_5ForConditionalGeneration.from_pretrained(
            args.repo, torch_dtype=dtype
        ).eval()
        processor = AutoProcessor.from_pretrained(args.repo)
        processor.tokenizer.save(str(args.out / "tokenizer.json"))
    ...
    print(f"[nuextract3] exporting vision encoder -> {enc_path}")
    ...
    print(f"[nuextract3] exporting streaming decoder -> {dec_path}")
    export_model_to_nnef(
        model=decoder,
        args=dec_args,
    ...
    if args.verify:
        ...
        print(f"[nuextract3] torch-ref token ids ({args.verify}): {ref}")
